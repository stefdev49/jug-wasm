...
wasmer_download_json() {
  ...
  if test -x "$(command -v curl)"; then
    response=$(curl -k -s -L -w 'HTTPSTATUS:%{http_code}' -H 'Accept: application/json' "$url")
...
wasmer_download_file() {
  ...
  if test -x "$(command -v curl)"; then
    if [ "$WASMER_INSTALL_LOG" = "$WASMER_VERBOSE" ]; then
      code=$(curl -k --progress-bar -w '%{http_code}' -L "$url" -o "$destination")
      printf "\033[K\n\033[1A"
    else
      code=$(curl -k -s -w '%{http_code}' -L "$url" -o "$destination")
    fi
  elif test -x "$(command -v wget)"; then
    if [ "$WASMER_INSTALL_LOG" = "$WASMER_VERBOSE" ]; then
